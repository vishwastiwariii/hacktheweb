...
// TODO: look the team up in the database once the teams table exists.
async function getTeam(): Promise<{ name: string } | null> {
  return null;
}
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-12">
      ...
      <section className="mt-6 rounded-lg border border-zinc-200 p-6 dark:border-zinc-800">
        ...
        {team ? (
          <p className="mt-3 text-lg font-medium">{team.name}</p>
        ) : (
          <>
            ...
            {/* TODO: wire these up on day 2. */}
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              <button
                type="button"
                className="h-11 rounded-lg bg-zinc-900 px-5 text-sm font-medium text-white transition-colors hover:bg-zinc-700 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200"
              >
                Create a team
              </button>
              <button
                type="button"
                className="h-11 rounded-lg border border-zinc-300 px-5 text-sm font-medium transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900"
              >
                Join a team
              </button>
            </div>
